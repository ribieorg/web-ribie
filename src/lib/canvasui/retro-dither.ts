/**
 * Canvas UI — Retro Dither (build vanilla TypeScript).
 * Fuente: https://canvasui.dev/docs/components/retro-dither
 * Licencia: MIT + Commons Clause (uso libre en producto propio; no revender la librería).
 *
 * Instalado A MANO, no con `shadcn add`: el CLI exige crear `components.json` y una
 * configuración de shadcn/Tailwind que este proyecto no tiene ni necesita. La propia
 * documentación contempla esta vía ("copy the file into your project, and import it").
 * Así entra sin una sola dependencia nueva.
 *
 * ⚠️ Requiere la API `html-in-canvas` (Chrome, tras `chrome://flags/#canvas-draw-element`)
 * y WebGL2. `supportsHtmlInCanvas()` permite decidir ANTES de montar nada: ver
 * `components/RetroDither.astro`, que solo construye los canvas si hay soporte.
 *
 * No editar a mano: si hay que actualizarlo, volver a copiarlo de la web.
 */
